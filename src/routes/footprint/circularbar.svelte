<script>
	export let value = 0;
	export let info = 'compared to the average human';
	export let color;
	...
	<div class="container">
		...
		{#if !checkable}
			<div class="info mx-auto text-center">
				<b class="px-auto text-4xl font-bold" bind:this={textLarge}>
					{actualValue}
				</b>
				<b bind:this={percent}>%</b>
				{#if info}
					<br />
    					<div class="text-center leading-tight">{info}</div>
				{/if}
			</div>
		{/if}
	</div>
